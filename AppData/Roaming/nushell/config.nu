...
def start-ssh-agent [] {
    # 检查 ssh-agent 是否可用
    let agent_status = (ssh-add -l | complete | get stdout | str trim)

    if $agent_status =~ "error" or $agent_status =~ "Could not connect" {
        # 启动用户级 ssh-agent（即使系统服务失败也能用）
        ^"ssh-agent" > nul
        sleep 100ms
    }

    # 再检查是否已加载私钥
    let keys = (ssh-add -l | complete | get stdout | str trim)
    if $keys =~ "The agent has no identities" {
        ssh-add ($env.USERPROFILE | path join ".ssh" "yiandrive")
    }
}
